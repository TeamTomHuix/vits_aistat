...
class LMCTS(object):
    ...
    def potential(self, theta, features, labels):
        ...
        return data_term + self.info["lambda"] * regularization_term
    ...
    def update_fct(self, key, context, action, reward, utils_vector):
        ...
        step_size = self.info['step_size'] / features.shape[0]

        key, subkey = jax.random.split(key)
        noises = np.sqrt(2 * step_size / self.info['eta']) * jax.random.normal(subkey, shape=(self.info["num_updates"], self.utils_object.dimension))
