...
class UtilsVector(object):
    ...
    def set(self, utils_vector, idx=None):
        if idx == None or self.info["agent"] == "ts" or self.info["agent"] == "ucb" or self.info['agent'] == "random":
            self.utils_vector = utils_vector
        else:
            if self.info["agent"] == "vits":
    ...
    def init_LinTS_LinUCB(self):
        ...
        cov = jnp.eye(self.info["d"]) / self.info["lambda"]
        return bt, cov

    def init_LMC(self):
        self.model = MLP(self.info["layers"], self.info['ctx_dim'],self.info["logistic_activation"])
    ...
    def init_VITS(self):
        ...
        cov_semi = jnp.eye(self.dimension) / jnp.sqrt(self.info["eta"] * self.info["lambda"])
        cov_semi_inv = jnp.eye(self.dimension) * jnp.sqrt(self.info["eta"] * self.info["lambda"])
    ...
    def init_RVITS(self):
        ...
        labels = jnp.zeros((self.info["T"]))
        mean = jnp.zeros((1, self.dimension))
        cov = jnp.eye(self.dimension) / (self.info["eta"] * self.info["lambda"])
    ...
    def init_VTS(self):
        ...
        labels = jnp.zeros((self.info["T"]))
        self.gamma_0 = self.info["gamma_0"] * jnp.ones((self.info["nb_arms"], self.info["nb_mixtures"]))
        self.alpha_0 = self.info["alpha_0"] * jnp.ones((self.info["nb_arms"], self.info["nb_mixtures"]))
        self.beta_0 = self.info["beta_0"] * jnp.ones((self.info["nb_arms"], self.info["nb_mixtures"]))
        self.Sigma_0 = jnp.tile(self.info["sigma_0"] * jnp.identity(self.info["ctx_dim"]), (self.info["nb_arms"], self.info["nb_mixtures"], 1, 1))
        self.Sigma_0_inv = jnp.tile((1 / self.info["sigma_0"]) * jnp.identity(self.info["ctx_dim"]), (self.info["nb_arms"], self.info["nb_mixtures"], 1, 1),)
        self.theta_0 = jnp.tile(jnp.expand_dims(self.info["theta_0"] * jnp.arange(0, self.info["nb_mixtures"], dtype=jnp.float32), (0, 2)), (self.info["nb_arms"], 1, self.info["ctx_dim"]))
